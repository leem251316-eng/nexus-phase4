...
QQQ_BEAR_RSI_GATE   = 58   # QQQ RSI must be above this for bear entry to be valid
                            # if QQQ is already oversold, reversal signal is weaker
    ...
    def check_reversal(self) -> bool:
        ...
        if state["state"] == "WATCHING":
            ...
            if drop >= REVERSAL_CONFIRM:
                ...
                # or at least neutral for a valid semi reversal
                if qqq_ctx.get("rsi", 50) < QQQ_BEAR_RSI_GATE:
                    log(self.symbol,
                        f"⚠️ REVERSAL WEAK: QQQ RSI={qqq_ctx['rsi']:.1f} < {QQQ_BEAR_RSI_GATE} "
...
def run():
    ...
    print("[PHASE4] NEXUS PHASE 4 V1.1 STARTING", flush=True)
