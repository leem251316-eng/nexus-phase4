        ...
        if res.status_code == 200:
            ...
            for asset in assets:
                if asset.get("currency") == "USD":
                    ...
                    # V1.4: Webull reports cash BP as 0 on some account types
                    # (debit card funded accounts use option_buying_power for
                    # the settled cash figure). Fall back if buying_power is 0.
...
def run():
    ...
    start_server(bots)
    alert("⚡ PHASE4 V1.3 ONLINE | NUGT+SOXL+LABU+TQQQ | QQQ filter active | Win-rate gate live")
