...
# Alpaca data client — replaces yfinance for 1-min/5-min bar fetching.
# yfinance returns empty DataFrames on Railway due to rate limiting and
# intraday data gaps; Alpaca IEX feed is reliable and already authenticated.
...
def fetch_prices_and_volumes(symbol: str, bars: int = 40, interval: str = "1m") -> tuple:
    """
    V2.1: Replaced yfinance with Alpaca StockHistoricalDataClient.
    yfinance returned empty DataFrames on Railway due to rate limiting and
    intraday data staleness. Alpaca IEX feed is real-time and already auth'd.
    Falls back to yfinance only if Alpaca data client is unavailable.
    """
    # Strip yfinance-style caret prefix (^VIX not valid for Alpaca)
    ...
    if _alpaca_data_client is not None:
        try:
            ...
            # Request enough history: 1-min needs 1 trading day, 5-min needs 3 days
            lookback  = timedelta(days=2) if interval == "1m" else timedelta(days=7)
            start_utc = now_utc - lookback
            ...
            bars_df = _alpaca_data_client.get_stock_bars(req).df

            if bars_df is not None and not bars_df.empty:
                # Multi-index: (symbol, timestamp) — drop symbol level
                if isinstance(bars_df.index, pd.MultiIndex):
                    bars_df = bars_df.xs(alpaca_sym, level="symbol")
                prices  = bars_df["close"].tail(bars).tolist()
                volumes = bars_df["volume"].tail(bars).tolist()
                if prices:
                    return prices, volumes
        except Exception:
            pass

    # Fallback: yfinance (less reliable on Railway but kept as safety net)
    try:
...
def get_current_price(symbol: str) -> float | None:
    """V2.1: Use Alpaca latest bar for current price; fall back to yfinance."""
    alpaca_sym = symbol.replace("^", "")
    if _alpaca_data_client is not None:
        try:
            req     = StockBarsRequest(
...
def run():
    ...
    alert(
        ...
        f"V1.9: Alpaca data feed replaces yfinance (fixes 0/60 warmup on Railway)"
    )
