"""
NEXUS PHASE 4 — PER-SYMBOL AUTONOMOUS BOTS V1.7
...
class SymbolBot:
    ...
    def compute_entry_score(self, sym: str, sym_ctx: dict) -> int:
        """
        V1.6: Per-symbol confluence score replacing the RSI-only gate.
        ...
        # Base positive signals — each worth +1
        all_positive = [
            "rsi_lt40", "rsi_lt25", "rsi14_lt35", "rsi14_lt20", "rsi21_lt45",
            "stochrsi_oversold", "near_lower_bb", "at_lower_bb", "far_below_bb",
            "bb_squeeze", "macd_bullish", "obv_rising", "obv_falling",
            "ema9_above_ema21", "bouncing", "williams_oversold", "cci_oversold",
            "below_ma20", "higher_lows",
        ]
        for sig in all_positive:
            if sig in best and sym_ctx.get(sig, False):
                score += 1
            elif sig not in worst and sym_ctx.get(sig, False):
                score += 1  # not in best but also not penalized
...
def run():
    ...
    print("[PHASE4] NEXUS PHASE 4 V1.6 STARTING", flush=True)
