"""
NEXUS PHASE 4 — PER-SYMBOL AUTONOMOUS BOTS V1.4
...
V1.4: Webull auth/balance diagnostics
  - get_buying_power()/get_account_id()/get_all_positions() no longer use
    bare "except: pass" -- failures are logged with the actual status code,
    response body, or exception, and the last error is tracked globally.
  - Boot-time buying-power check: alerts via Telegram immediately if Webull
    auth/balance is broken at startup, instead of bots silently sitting
    idle for hours with zero log output (the V1.3 failure mode).
  - try_buy()'s "trade_size < $1" early-return now alerts (rate-limited to
    1/hour) and distinguishes "balance read is broken" from "balance is
    just genuinely too small right now for this bot's allocation".
V1.4.1: One-time full USD asset dump (via Telegram) on first successful
  balance read, to diagnose buying_power=0 vs other balance fields
  (cash_balance, settled_funds, etc.) that Webull's API may return.
"""
...
PM_MIN_BUCKET_TRADES = 3    # min samples per bucket before it's used (see run_analysis)

# V1.3: Win-rate gate -- skip entries whose exact historical bucket has a
# win rate below this. Only applies once a bucket has >= PM_MIN_BUCKET_TRADES
# samples (see Phase4Memory.should_skip_entry).
...
def get_account_id() -> str:
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            print("[PHASE4] 🔴 get_account_id: 200 OK but empty account list", flush=True)
        else:
            print(f"[PHASE4] 🔴 get_account_id: HTTP {res.status_code}: {res.text[:200]}", flush=True)
    except Exception as e:
        print(f"[PHASE4] 🔴 get_account_id: exception: {e}", flush=True)
    return ""

_balance_cache      = {}
_balance_cache_time = 0.0
_balance_last_error = None   # V1.4: last failure reason, for diagnostics/alerts
_balance_ok_ever    = False  # V1.4: has a real balance read ever succeeded?
_balance_dumped     = False  # V1.4.1: dump full USD asset dict once for diagnostics
_low_bp_alert_time  = 0.0    # V1.4: last time the "trade_size too small" alert fired
LOW_BP_ALERT_INTERVAL = 3600 # V1.4: don't re-alert more than once per hour

def get_buying_power(acct_id: str) -> float:
    global _balance_cache, _balance_cache_time, _balance_last_error, _balance_ok_ever, _balance_dumped
    now = time.time()
        ...
        if res.status_code == 200:
            ...
            for asset in assets:
                if asset.get("currency") == "USD":
                    ...
                    _balance_last_error = None
                    _balance_ok_ever    = True
                    if not _balance_dumped:
                        _balance_dumped = True
                        print(f"[PHASE4] 💰 USD asset (full): {asset}", flush=True)
                        alert(f"💰 PHASE4 USD asset fields:\n{asset}")
                    return float(asset.get("buying_power", 0))
            # V1.4: 200 OK but no USD asset in response -- log the shape we got
            _balance_last_error = (
                f"200 OK but no USD asset in account_currency_assets "
                f"(got currencies: {[a.get('currency') for a in assets]})"
            )
            print(f"[PHASE4] 🔴 get_buying_power: {_balance_last_error}", flush=True)
        elif res.status_code == 429:
            _balance_last_error = "429 rate limited"
            time.sleep(WEBULL_429_BACKOFF)
        else:
            _balance_last_error = f"HTTP {res.status_code}: {res.text[:200]}"
            print(f"[PHASE4] 🔴 get_buying_power: {_balance_last_error}", flush=True)
    except Exception as e:
        _balance_last_error = f"exception: {e}"
        print(f"[PHASE4] 🔴 get_buying_power: {_balance_last_error}", flush=True)
...
def get_all_positions(acct_id: str) -> dict:
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            print(f"[PHASE4] 🔴 get_all_positions: HTTP {res.status_code}: {res.text[:200]}", flush=True)
    except Exception as e:
        print(f"[PHASE4] 🔴 get_all_positions: exception: {e}", flush=True)
...
class Phase4Memory:
    ...
    def should_skip_entry(self, symbol: str, is_bear: bool, mode: str,
                           ...
                           hour: int) -> tuple:
        """
        V1.3: Win-rate gate. Returns (skip, win_rate, has_data).

        has_data is True only if this exact bucket has >= PM_MIN_BUCKET_TRADES
        (3) historical samples -- run_analysis() only writes buckets that meet
        that threshold, so presence in _win_rates IS the sample-size check.
        When has_data is False, win_rate is returned as 0.5 (neutral) and
        skip is always False -- "no data" never blocks a trade, it just
        doesn't help one either.
        """
        key = self._bucket_key(symbol, is_bear, mode, sym_rsi, spy_ctx, qqq_ctx, hour)
...
class SymbolBot:
    ...
    def try_buy(self, sym: str, prices: list, spy_ctx: dict, sym_ctx: dict) -> bool:
        ...
        global _low_bp_alert_time
        bp         = get_buying_power(self.acct_id)
        trade_size = round(bp * self.budget_pct, 2)
        if trade_size < 1.00:
            # V1.4: this used to be a silent return -- now we distinguish
            # "balance read is broken" (bp==0 and we've never had a good
            # read, or last read errored) from "balance is just genuinely
            # too small for this bot's allocation right now", and alert
            # (rate-limited to once/hour) so this state is never invisible
            # for hours like it was before.
            now = time.time()
            if now - _low_bp_alert_time > LOW_BP_ALERT_INTERVAL:
                _low_bp_alert_time = now
                if not _balance_ok_ever or (bp <= 0 and _balance_last_error):
                    alert(
                        f"🔴 PHASE4: {self.symbol} can't trade -- buying power "
                        f"read as ${bp:.2f} ({_balance_last_error or 'no successful read yet'})"
                    )
                else:
                    alert(
                        f"⚠️ PHASE4: {self.symbol} idle -- buying power ${bp:.2f} "
                        f"x {self.budget_pct:.0%} budget = ${trade_size:.2f} < $1 minimum"
                    )
            return False
        ...
        # V1.3: Win-rate gate -- skip entries where this exact historical
        # setup (symbol|bear-or-bull|mode|RSI bucket|SPY trend|QQQ trend|hour)
        # has a win rate below WIN_RATE_GATE_THRESHOLD, once enough samples
        # exist (>= PM_MIN_BUCKET_TRADES). "No data" never blocks a trade.
        qqq_ctx = get_qqq_context()
        if _phase4_memory:
            ...
            if has_data:
                log(self.symbol,
                    f"✅ Win-rate check passed: {sym} | mode={self.mode} | "
                    f"historical WR={wr:.0%}")
...
def run():
    ...
    print("[PHASE4] NEXUS PHASE 4 V1.4 STARTING", flush=True)
    print("[PHASE4] Bots: NUGT(30%) | SOXL(25%) | LABU(25%) | TQQQ(20%)", flush=True)
    print("[PHASE4] Bear pairs: DUST | SOXS | LABD | SQQQ", flush=True)
    print("[PHASE4] V1.1: QQQ filter | Pattern memory | MFE/MAE tracking | V1.4: Webull auth diagnostics", flush=True)
    ...
    if not acct_id:
        ...
        alert("🔴 PHASE4 BOOT FAILED: Could not get Webull account ID — check WEBULL_APP_KEY/SECRET env vars")
        return

    print(f"[PHASE4] Account: {acct_id}", flush=True)

    # V1.4: boot-time buying power check -- if Webull auth/balance is broken,
    # surface it immediately via Telegram instead of bots silently sitting
    # idle for hours with trade_size < 1.00 and no log output at all.
    boot_bp = get_buying_power(acct_id)
    if _balance_ok_ever and boot_bp > 0:
        print(f"[PHASE4] ✅ Buying power check OK: ${boot_bp:.2f}", flush=True)
    elif _balance_ok_ever and boot_bp <= 0:
        print(f"[PHASE4] ⚠️ Buying power read OK but is ${boot_bp:.2f} — bots may be capital-constrained", flush=True)
        alert(f"⚠️ PHASE4 BOOT: Webull buying power is ${boot_bp:.2f} — bots will be idle until funds free up")
    else:
        print(f"[PHASE4] 🔴 Buying power check FAILED: {_balance_last_error}", flush=True)
        alert(
            f"🔴 PHASE4 BOOT: Webull buying power check failed -- "
            f"{_balance_last_error}\n"
            f"Bots will appear idle/silent until this is fixed (likely "
            f"Webull API auth/token issue -- check app re-authorization)."
        )
    ...
    start_server(bots)
    alert("⚡ PHASE4 V1.4 ONLINE | NUGT+SOXL+LABU+TQQQ | QQQ filter active | Win-rate gate live | Webull auth diagnostics")
